...
int __init spi_register_board_info(struct spi_board_info const *info, unsigned n)
{
	...
	for (i = 0; i < n; i++, bi++, info++) {
		...
		list_for_each_entry(master, &spi_master_list, list)   /* ±Èspi_register_masterÖÐÏÈÔËÐÐ*/
...
int spi_register_master(struct spi_master *master)
{
	...
	list_add_tail(&master->list, &spi_master_list);   /* ±Èspi_register_board_infoÖÐºóÔËÐÐ*/
#if 1
